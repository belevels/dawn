...
class CarouselComponent extends SlideshowComponent { 
  ...
  autoRotateSlides() {
    ...
    if (window.matchMedia("(min-width: 990px)").matches) {
      var slideScrollPosition = this.currentPage === (this.sliderItems.length - this.TotalSlidesDesktop.textContent - 0) ? 0 : this.slider.scrollLeft + this.slider.querySelector('.slideshow__slide').clientWidth; // desktop
    } else if (window.matchMedia("(min-width: 750px) and (max-width: 989px)").matches) {
...
window.addEventListener("DOMContentLoaded", function() {
  ...
  var cantMin = parseFloat(cantMinText.textContent.match(/(-\d+|\d+)(,\d+)*(\.\d+)*/g)[0].replaceAll(',', '.'));
  var discountText = document.querySelector("div.be-box > div.aiod-volume-discount-wrap > div > table > tbody > tr > td:nth-child(2)");
  var discount = parseFloat(discountText.textContent.match(/(-\d+|\d+)(,\d+)*(\.\d+)*/g)[0].replaceAll(',', '.')) / 100;

  var priceRegWithDecimals = Math.floor(priceReg * 100) / 100.0 * cantMin;
  var priceEcoPack = priceReg - (priceReg * discount);
  var priceEcoPackwithDecimals = Math.floor(priceEcoPack * 100) / 100.0 * cantMin;
  console.log('priceEcoPack = ' + priceEcoPack);
  console.log('priceEcoPackwithDecimals = ' + priceEcoPackwithDecimals);
